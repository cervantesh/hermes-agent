...
|---|---|---|---|
| The published branch is rebased onto the refreshed upstream main without patch drift | exact base/head inspection plus `git range-diff` from the previously reviewed series to the rebased series | **confirmed** | base `b954547e726eb8df7479178d3db489852904705b`; head `99daedd99928c26ba0259c54d292fb790228b71a`; all three patches `=` |
...
| Focused compatibility gate is green | Focused `pytest` command in `REPRODUCE.md` | **confirmed** | `56 passed in 8.15s` on exact rebased head |
| Historical single-process diagnostic behavior is equivalent to clean main | Same direct `python -m pytest` selection on candidate and detached clean main | **confirmed within that diagnostic** | candidate `610/42/42`; main `607/42/42`; identical inherited node outcomes. This was not the canonical per-file runner and is not a current-CI result. |
| The inherited diagnostic failures have independent ownership | Canonical per-file follow-up on clean main plus causal controls | **confirmed** | `598 passed`, `12 failed`, `42 skipped`, plus one passing file exceeding the 300-second per-file limit; tracked separately in [#98037](https://github.com/NousResearch/hermes-agent/issues/98037), [#98038](https://github.com/NousResearch/hermes-agent/issues/98038), [#98039](https://github.com/NousResearch/hermes-agent/issues/98039), and [#98040](https://github.com/NousResearch/hermes-agent/issues/98040). None is caused by this refactor. |
...
| The exact rebased head preserves the real native-supervisor restart boundary on Linux, macOS, and Windows | Before/after ephemeral services on systemd, launchd, and Windows SCM; require discovery, real restart, PID rotation, supervised/running post-state, cleanup, and identical contracts | **confirmed within supervisor-boundary scope** | base `b954547e726eb8df7479178d3db489852904705b`; head `99daedd99928c26ba0259c54d292fb790228b71a`; all three comparators `equivalent`; every contract assertion true; [run 33260176395](https://github.com/cervantesh/hermes-agent/actions/runs/33260176395) |
